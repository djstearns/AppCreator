 ...
 */
class BraintreeController extends BraintreeAppController {
 ...
 */
	function admin_buildnewmobilefiles(){
		...
		$schema =& new CakeSchema(array('name'=>'DsSchema','plugin'=>'Braintree'));
		...
		$alloystr3 = "Alloy.Globals.PLUGIN = 'braintree/';";
		...
		$alloystr2 .= '};
		';
		$this->bakeMobileAlloyFile("Alloy.Globals.BASEURL = 'http://www.derekstearns.com/';".$alloystr, $alloystr2, $alloystr3);
	...
	function admin_downloadmobile(){
		$source_dir = rtrim(getcwd (),'webroot').'Plugin/Braintree/webroot/mobile/';
		$dest_dir =  rtrim(getcwd (),'webroot').'Plugin/Braintree/webroot/';
		$zip_file = 'Braintreemobile'.date('Ymd').'.zip';
 
		if($this->Zip($source_dir, $dest_dir.$zip_file)!=false){
	
			$this->redirect('http://www.derekstearns.com'.'/braintree/'.$zip_file);
		}
	}
	...
	function bakeMobileAlloyFile($alloystr , $alloystr2, $alloystr3){
		...
		$totalalloystr .= "//:::::::APP CREATOR SYNC OPTIONS:::::::
			...
			$pathx = rtrim(getcwd (),'webroot').'Plugin/Braintree/webroot/mobile/app/';
	...
	function bakeMobileModel($name, $data = array()) {
	  $relationstr = '';
	   ...
	   if (is_object($name)) {
		...
		$pathx = rtrim(getcwd (),'webroot').'Plugin/Braintree/webroot/mobile/app/models/';
   ...
   function bakeMobileController($name, $data = array()){
	...
	$modelArr =  $array = json_decode(json_encode($name->schema()), true);
		...
		*/
		...
		";
		//create new M2M controller
		$pathx = rtrim(getcwd (),'webroot').'Plugin/Braintree/webroot/mobile/app/controllers/';
   ...
   function bakeMobileViews($name, $data = array()){
		...
		*/
		...
		$pathx = rtrim(getcwd (),'webroot').'Plugin/Braintree/webroot/mobile/app/views/';
