 ...
 */
	public function create(&$model, $fields = null, $values = null) {
		...
		try {
			switch ($entity) {
				case 'Customer':
					$result = BraintreeCustomer::create($to_save);
					if (!$result->success) {
				...
				case 'Transaction':
					...
					if (
						...
						$result = BraintreeTransaction::refund($braintree_transaction_id, $to_save['amount']);
					} elseif (
						!empty($to_save['type']) && 
						...
						$result = BraintreeTransaction::sale(array_merge(
							array(
								'options' => array(
 ...
 */
	public function update(&$model, $fields = null, $values = null) {
		...
		try {
			switch ($entity) {
				case 'Customer':
					$result = BraintreeCustomer::update($model->id, $to_save);
					break;
				case 'Transaction':
					$transaction = $this->read($model, array(
						'conditions' => array(
					...
					if (!empty($to_save['status']) && $to_save['status'] == 'voided') {
						...
						$result = BraintreeTransaction::void($braintree_transaction_id);
						if (!$result->success) {
							$this->showError($result->message);
							return false;
						}
						...
						if (!empty($to_save['amount'])) {
							$result = BraintreeTransaction::submitForSettlement($braintree_transaction_id, $to_save['amount']);
						} else {
							$result = BraintreeTransaction::submitForSettlement($braintree_transaction_id);
						}
						if (!$result->success) {
							$this->showError($result->message);
							return false;
 ...
 */
	public function read (&$model, $queryData = array()) {
			...
			try {
				switch ($entity) {
					case 'Customer':
						...
						$customer = BraintreeCustomer::find($conditions[$model->alias . '.' . $model->primaryKey]);
						$result = array(
							0 => array(
								$model->alias => array(
									'id' => $customer->id,
					...
					case 'Transaction':
						...
						$transaction = BraintreeTransaction::find($braintree_transaction_id);
						$result = array(
							0 => array(
								$model->alias => array(
						...
						break;
					case 'CreditCard':
						$credit_card = BraintreeCreditCard::find($conditions[$model->alias . '.' . $model->primaryKey]);
						$result = array(
							0 => array(
								$model->alias => array(
					...
					case 'Address':
						...
						$address = BraintreeAddress::find($customer_id, $address_id);
						$result = array(
							0 => array(
								$model->alias => array(
		...
		if (empty($conditions)) {
			try {
				$all_customers = BraintreeCustomer::all();
			} catch (Exception $e) {
				$this->showError(print_r($e, true));
				return array();
			}
				...
				try {
					switch ($entity) {
						...
						case 'Address':
							...
							list($customer_id, $address_id) = $exploded;
							BraintreeAddress::delete($customer_id, $address_id);
							break;
						default:
							return false;
							break;
