 ...
 */
	public function beforeSave () {
		...
		$result = Braintree_CreditCard::fromNonce($this->data['BraintreeCreditCard']['unique_card_identifier']);
		$result = $result->_attributes;
		$this->data['BraintreeCreditCard']['card_type'] = $result['cardType'];
		$this->data['BraintreeCreditCard']['token'] = $result['token'];
		$this->data['BraintreeCreditCard']['is_default'] = $result['default'];
		$this->data['BraintreeCreditCard']['expiration_date'] = $result['expirationDate'];
		$this->data['BraintreeCreditCard']['masked_number'] = $result['maskedNumber'];
		$this->data['BraintreeCreditCard']['expiration_date'] = $result['expirationYear'].'-'.$result['expirationMonth'].'-01';
		
		$exists = $this->find('first', array('conditions'=>array('token'=>$this->data['BraintreeCreditCard']['token'])));
		if(!empty($exists['BraintreeCreditCard'])){
			$this->data['BraintreeCreditCard']['id'] = $exists['BraintreeCreditCard']['id'];
		}
